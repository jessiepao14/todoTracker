...
function insertConfig(path, state, mode, context) {
    if (state.opts.mode === 'compile-time') {
        ...
        let literalResult = evaluate_json_1.buildLiterals(collapsed.config, context);
...
function inlineRuntimeConfig(path, state, context) {
    path.get('body').node.body = [
        context.types.returnStatement(evaluate_json_1.buildLiterals({ packages: state.opts.userConfigs, global: state.opts.globalConfig }, context)),
    ];
}
